...
if page == "🔮 Prediksi Biaya":
    st.title("🔮 Prediksi Biasi Asuransi")
    ...
    with col2:
        ...
        if 'hasil' in st.session_state:
            st.metric(label="Total Estimasi Tagihan", value=f"${st.session_state.hasil:,.2f}")
            if perokok == "Ya":
...
# --- HALAMAN 2: VISUALISASI ---
    ...
    with c2:
        ...
        korelasi = pd.DataFrame({'Age': [1.0, 0.1, 0.3], 'BMI': [0.1, 1.0, 0.2], 'Charges': [0.3, 0.2, 1.0]}, index=['Age', 'BMI', 'Charges'])
